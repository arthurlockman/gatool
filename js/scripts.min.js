...
function getEventAwards() {
    "use strict";
    var teamNumberList = {};
    var req = new XMLHttpRequest();
    var endpoint = "/eventAwards/";

    for (var i = 0; i < eventTeamList.length; i++) {
        teamNumberList.teams.push(eventTeamList[i].teamNumnber);
    }
    teamNumberList.awardDepth = localStorage.awardDepth;
    req.open('POST', apiURL + endpoint + localStorage.currentYear + '/');
    req.addEventListener('load', function () {
        var data = JSON.parse(req.responseText);
        if (data.teamList.length > 0) {
            for (var i = 0; i < data.teamList.length; i++) {
                var awardText = "";
                var team = JSON.parse(localStorage['teamData'+data.teamList[i].teamNumber]);
                for (var j = 0; j < data.teamList[i].awards.length; j++) {
                    awardText += data.teamList[i].awards[j];
                    if (j<data.teamList[i].awards.length-1){
                        awardText+=localStorage.awardSeparator;
                    }
                }
                team.awards = awardText;
                localStorage['teamData'+data.teamList[i].teamNumber]=JSON.stringify(team);
            }
        }

    });
    req.send(JSON.stringify(teamNumberList));
}
